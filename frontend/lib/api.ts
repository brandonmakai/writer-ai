...
function parseTokensUsed(res: Response): number | null {
  const raw = res.headers.get("X-Tokens-Used");
  if (raw === null) return null;
  const n = parseInt(raw, 10);
  return Number.isNaN(n) ? null : n;
}
...
export async function fetchOutline(body: OutlineRequest): Promise<{
  ...
  return { outline, remainingAttempts: parseRemainingAttempts(res), resetIn: parseResetIn(res), tokensUsed: parseTokensUsed(res) };
}
...
export async function fetchRewrite(body: RewriteRequest): Promise<{
  ...
  return { rewrite, remainingAttempts: parseRemainingAttempts(res), resetIn: parseResetIn(res), tokensUsed: parseTokensUsed(res) };
}
...
export async function fetchEdit(body: EditRequest): Promise<{
  ...
  return { edit, remainingAttempts: parseRemainingAttempts(res), resetIn: parseResetIn(res), tokensUsed: parseTokensUsed(res) };
}
